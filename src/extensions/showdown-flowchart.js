...
function renderFlowchart(element, options, sync) {
  ...
  const id =
    'flowchart-' + Date.now() + '-' + Math.floor(Math.random() * 10000);
  ...
  return [
    {
      ...
      filter: function(html) {
        ...
        const flowchartElements = wrapper.querySelectorAll(
          'code.flowchart.language-flowchart'
        );
        const flowElements = wrapper.querySelectorAll(
          'code.flow.language-flow'
        );

        if (
          !renderFlowchartElements(flowchartElements, flowElements, options)
        ) {
          return html;
        }
