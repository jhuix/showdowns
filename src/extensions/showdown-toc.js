...
function createHeadingElement(wrapper, element, toc, headingLevel, nexthead) {
  try {
    if (nexthead) {
      ...
      if (title) {
        ...
        if (icon) {
          ...
          icon.innerHTML = '<svg class="icon"><use xlink:href="#icon-expand"></use></svg>';
        }
      }
    ...
    if (element) {
      ...
      icon.innerHTML = '<svg class="icon"><use xlink:href="#icon-chapter-l"></use></svg>';
...
function renderTocElements(wrapper, config) {
  ...
  const elements = wrapper.querySelectorAll(heading.join(',') + (config.compatible ? ',p' : ''));
  for (let i = 0; i < elements.length; i++) {
    ...
    if (test.test(element.textContent.trim())) {
      ...
      if (!firstHeadOne) {
        ...
        let tocToggle = i18n.getLangString('toc-toggle-prefix');
        ...
        if (config.expand) {
          swtichToc.classList.add('hidden');
        } else {
          showdownToc.classList.add('hidden');
        }
      } else {
        showdownToc = wrapper.ownerDocument.createElement('div');
        showdownToc.classList.add('showdown-toc');
      ...
      tocView.classList.add('toc-view');
    ...
    if (heading.includes(element.tagName)) {
      if (!firstHeadOne) {
        firstHeadOne = true;
      }
