...
function parseAttribute(attribute, exclude) {
  ...
  attrs.forEach((attr) => {
    if (attr) {
      if (attr[0] === '#') {
        obj.ids.push(attr.substring(1));
      } else if (attr[0] === '.') {
        if (exclude) {
          const svgType = parseSvgType(attr.substring(1));
          if (svgType) {
            obj.svgType = svgType;
          }
        }
        if (!exclude || (attr !== exclude && !attr.startsWith(exclude + '-'))) {
          obj.classes.push(attr.substring(1));
        }
      } else {
        const kv = attr.split('=');
        if (kv.length === 2) {
  ...
  return [
    {
      ...
      filter: (text, converter) =>
        text.replace(
          /^ {0,3}(:::+)(?: +([\S]+)[ \t]*([^\s][\S\t ]*)|([^\[\]\{\}\s]+)(?:\[([ \t\S]*)\]){0,1}(?:\{([ \t\S]*)\}){0,1})\n([\s\S]*?)\n {0,3}\1[ \t]*$/gm,
          function (wholeMatch, delim, classname0, title0, name, title, attribute, content) {
            const classnames = ['showdown-container'];
            let id = '';
            let style = '';
            let svgContent = '';
            if (classname0) {
              const classes = classname0.split('-');
              ...
              switch (name) {
                case 'note':
                case '备注':
                  obj = parseAttribute(attribute, '.alert');
                  classnames.push('note');
                  break;
                case 'alert':
                case '注意':
                  classnames.push('alert');
                  obj = parseAttribute(attribute, '.note');
                  break;
                default:
                  obj = parseAttribute(attribute);
                  obj.svgType = parseSvgType(name);
              }
              if (!obj.svgType) {
                obj.svgType = { id: 'icon-pro-note', type: 'success' };
              }

              if (!obj.classes.includes('alert') && !classnames.includes('alert')) {
                if (!obj.classes.includes('note') && !classnames.includes('note')) {
                  classnames.push('note');
                }
              }
              if (obj.classes.length > 0) {
                classnames.push(...obj.classes);
              }
              if (!obj.classes.includes(obj.svgType.type)) {
                classnames.push(obj.svgType.type);
              }
              if (obj.ids.length > 0) {
                id = `id="${obj.ids.join(' ')}" `;
              }
              ...
              svgContent = `<span class="container-icon"><svg xmlns="http://www.w3.org/2000/svg"><use xlink:href="#${obj.svgType.id}"/></svg></span>`;
            }
            if (title) {
              title = converter.makeHtml(title).replace(/^<p>(.*)<\/p>$/, '$1');
              title = `<div class="container-title">${svgContent}${title}</div>`;
            } else {
              title = ''
            }
            if (content) {
              content = `<div class="container-content">${converter.makeHtml(content)}</div>`;
            } else {
              content = '';
            }
            return `<div ${id}class="${classnames.join(' ')}"${style}>${title}${content}</div>`;
          }
        ),
    },
  ];
...
function showdownAsyncContainer() {
  return [
    {
      type: 'output',
      filter: function (obj) {
...
export { showdownContainer as default, showdownContainer, showdownAsyncContainer };
